...
public class MD5Util {
	...
	public final static String encodeStr(String str){
		return MD5Util.byte2hex(MD5Util.encode(str.getBytes())).toUpperCase();
	}
	/**
	 * MD5编码
	 * @param origin 原始字符串
...
//			strMd5 = MD5Util.byte2hex(MD5Util.encode(str.getBytes("GBK")));
			strMd5 = MD5Util.byte2hex(MD5Util.encode(str.getBytes(encode)));
		}catch (Exception ex){
			ex.printStackTrace();
		}
		return strMd5;
...
//		pwdHash = new String(Base64Util.encode(bytes)); //4.
		System.out.println(MD5Util.byte2hex(MD5Util.encode(pwd.getBytes("GBK"))));
